...
def process_entities_raw(game_dir, entities_raw, force_recompile, search_paths, vpk_paths_from_gameinfo):
    entities_ready = []
    entities_todo = []
    ...
    print_and_log(f" ")
    print_and_log(f"Checking if there are models with scale 1 that are already static_prop...")
    ...
    for entity in entities_raw:
        ...
        ###!!! ХОТЯ ВОЗМОЖНО лучше эту проверку добавлять не здесь, а там же, где мы уже обрабатываем модельки, просто чтобы дважды одну и ту же работу не делать, всё равно там просматриваются все неотброшенные модели, перед декомпиляцией легко будет вычислить что декомпиляция даже не нужна!!!

        """
        hammer_mdl_path = model
        if debug_mode: print_and_log(f"hammer_mdl_path: {hammer_mdl_path}")
        
        real_mdl_path = find_real_mdl_path(game_dir, hammer_mdl_path)

        if real_mdl_path:
            # если моделька нашлась в файлах мода, здесь можно сразу прочитать заголовок её .mdl
            #real_mdl_paths.append(real_mdl_path)
        else:
            if debug_mode: print_and_log(f" ")
            if debug_mode: print_and_log(f"{mdl_name}.mdl not found in project content, trying to find in paths from GameInfo...")
            ...
            if mdl_path_from_other_contents != None:
                # если нашли модельку в кастомных папках - тоже можем сразу прочитать заголовок её .mdl 
                # real_mdl_paths.append(mdl_path_from_other_contents)
                if debug_mode: print_and_log(Fore.GREEN + f"{mdl_name}.mdl found!")
            else:
                if debug_mode: print_and_log(f"{mdl_name}.mdl not found in paths from gameinfo.txt")
                if debug_mode: print_and_log(f"Trying to find {mdl_name}.mdl in vpks...")
    
                # нужны vpkeditcli_path, vpk_extract_folder?
                
                extracted_mdl_path = extract_mdl(vpkeditcli_path, hammer_mdl_path, vpk_extract_folder, vpk_files, only_mdl=True)
                if debug_mode: print_and_log(Fore.YELLOW + f"extracted_mdl_path: {extracted_mdl_path}")

                if extracted_mdl_path != None:
                    # если нашлась моделька где-то среди указанных .vpk
                    #real_mdl_paths.append(extracted_mdl_path)
                    
                    # нужно будет обязательно удалять этот extracted .mdl файл если он нашёлся!!!!
                    # т.к. возможно искалка далее в обработчике решит что эта моделька уже полностью экстрактнута, а не только .mdl
                    
                else:
                    # если моделька ваще нигде не нашлась, в этом случае мы добавляем её в entities_ready, т.к. она и 
...
def extract_mdl(vpkeditcli_path, hammer_mdl_path, vpk_extract_folder, vpk_files, only_mdl=False):
    ...
    if vpk_with_mdl != None:
        ...
        try:
            ...
            if not only_mdl: extract_paths.append(mdl_folder_path + mdl_name + ".dx80.vtx")
            if not only_mdl: extract_paths.append(mdl_folder_path + mdl_name + ".dx90.vtx")
            if not only_mdl: extract_paths.append(mdl_folder_path + mdl_name + ".sw.vtx")
            if not only_mdl: extract_paths.append(mdl_folder_path + mdl_name + ".vvd")
            if not only_mdl: extract_paths.append(mdl_folder_path + mdl_name + ".phy")
...
def entities_todo_processor(entities_todo, entities_ready, ccld_path, gameinfo_path, compiler_path, game_dir, convert_to_static, subfolders, vpkeditcli_path):
    ...
    search_paths = get_search_paths(game_dir)
...
def main():
    ...
    entities_ready, entities_todo = process_entities_raw(game_dir, entities_raw, force_recompile, search_paths, vpk_paths_from_gameinfo)
