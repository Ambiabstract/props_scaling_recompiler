...
`test_staging_toolchain.py` проверяет operation-scoped staging root, защиту от path escape/conflicting writes, повторную сверку provenance/hash перед materialization, безопасный cleanup, argv-only adapters Crowbar/StudioMDL и обязательную post-compile проверку managed path, MDL header/static flag и полного набора SDK 2013 SP companions. Отдельные cases фиксируют реальное 63-байтовое обрезание длинного `studiohdr_t::name[64]` и отклонение неверного префикса. Встроенная QC-матрица перекрёстно покрывает static/dynamic, отсутствие collision/`$collisionmodel`/`$collisionjoints` и наличие/отсутствие исходного `$scale` без обращения к внешнему SDK.

MDL/bodygroup cases в `test_srctools_mdl.py`, `test_vmf_discovery_planning.py`, `test_qc_transformer.py`, `test_skin_layout.py`, `test_generation_pipeline.py` и `test_vmf_output.py` фиксируют runtime-safe fallback 2.0: dynamic MDL с пустым option многовариантного bodygroup вообще не компилируется и не участвует в VMT/skin-layout/reconciliation. VMF получает исходные `model`, `modelscale`, `rendercolor` и `skin`, classname `prop_dynamic` и не содержит legacy `convert_prop_to_static`. Попытка static-conversion такого QC завершается fail-closed; zero-triangle placeholder API удалён. `test_srctools_vmt.py` отдельно фиксирует общую нормализацию `SDK_<Shader> -> <Shader>` при выборе color-policy: `SDK_VertexLitGeneric` и `SDK_UnlitGeneric` получают `$color2`, а `SDK_LightmappedGeneric` остаётся неподдерживаемым вместе с базовым shader.
...
`test_cli.py` фиксирует основные/deprecated аргументы, `-debug_cleanup`, порядок project/session/elapsed summary, поиск отдельного Crowbar и no-op CLI. `test_frozen_executable.py` по явному `PSR_FROZEN_EXE=<path>` запускает настоящий one-file exe в полностью временном проекте без Python/Crowbar/StudioMDL, проверяет `vmf_out`, LocalAppData manifest, deprecated warning и жёсткий предел 64 MiB; отдельно фиксирует v3 icon и исключение неиспользуемых native/runtime modules. По умолчанию frozen run пропускается.

`test_external_sdk_generation.py` по явному `PSR_RUN_EXTERNAL_SDK=1` запускает read-only/isolated matrix на реальном `book_2`, Crowbar 0.68 и StudioMDL SDK 2013 SP. Direct `insert`, direct `replace` и source-Patch full-copy cases используют только temporary staging/overlays; compiled QC дополнительно проверяется на отдельный `$cdmaterials "models/psr_scaled/"` и относительные managed skin names, чтобы исключить runtime path вида `models/<source-dir>/models/psr_scaled/...`. Capacity cases доказывают успешные 31 material/1024 family rows и отказ StudioMDL на 32 material/1025 rows. Default test run пропускает их; протокол границ находится в `docs/research/SDK_SKIN_LIMITS.md`.
