...
`test_smd_generation.py` и bodygroup cases в `test_qc_transformer.py`/`test_generation_pipeline.py` фиксируют узкий workaround crash StudioMDL: при dynamic-to-static conversion первый `blank` option заменяется staging-only zero-triangle SMD с сохранёнными nodes/skeleton. `test_srctools_vmt.py` отдельно фиксирует общую нормализацию `SDK_<Shader> -> <Shader>` при выборе color-policy: `SDK_VertexLitGeneric` и `SDK_UnlitGeneric` получают `$color2`, а `SDK_LightmappedGeneric` остаётся неподдерживаемым вместе с базовым shader.
