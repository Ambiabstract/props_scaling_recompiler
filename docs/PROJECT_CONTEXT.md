...
- Добавлены argv-only adapters Crowbar и StudioMDL без `shell=True`, с timeout, byte-preserving stdout/stderr и категоризированными ошибками запуска/exit code. Crowbar считается успешным только при одном однозначном QC в предварительно пустом output directory. StudioMDL отделён от post-compile validation: generated model проверяется по managed path, обязательным `.mdl/.vvd/.dx90.vtx`, optional `.dx80.vtx/.sw.vtx`, PHY при наличии collision, MDL version/internal name и static-prop flag, а не по наличию строки `Completed` в логе.
...
- Добавлен production runtime coordinator и CLI entry point `psr_entrypoint.py`: полный запуск связывает discovery, planning, reconciliation, generation, validation и commit, а no-op создаёт эквивалентный `vmf_out` без Crowbar/StudioMDL. Постоянный project state под `%LOCALAPPDATA%\PropsScalingRecompiler\projects\<project_id>` содержит manifest, lock и recovery journal; короткий временный staging находится в соседнем managed `work`, а отчёты — в `logs/<project>--<short_id>/<map>__<UTC time>__pid-<pid>.log`. OS-level project lock запрещает два одновременных compile-run одного проекта и автоматически освобождается после завершения процесса.
...
- Общий compile-failure outcome выбирается `-compile_failure_mode 0/1/2/3`: удалить entity; записать `prop_static` с предназначенным, но отсутствующим generated path и source skin (default `1`); использовать `prop_dynamic_override`; использовать `prop_scalable`. Runtime-варианты сохраняют исходные `model/modelscale/rendercolor/skin`, static-вариант удаляет runtime scale/color и пишет плановый model/skin. Особый подтверждённый empty-bodygroup путь остаётся независимым `prop_dynamic`, но отсутствие static-result показывается красным `ERROR`.
...
Общий исход выбирается художником через `-compile_failure_mode`: `0` удаляет entity из `vmf_out`; `1` (default) пишет `prop_static` с тем generated model path и исходным skin, которые использовались бы при успехе, даже если файла нет; `2` пишет `prop_dynamic_override`; `3` пишет `prop_scalable`. Режимы `2/3` оставляют исходные `model`, `modelscale`, `skin`, `rendercolor` и прочие runtime properties, удаляя только служебные PSR keys. Режим `1` удаляет `modelscale/rendercolor` как успешный static output и намеренно оставляет движку видимую missing-model диагностику. Любой из исходов сохраняет красный `ERROR` в итоговом отчёте.
...
- Общий compile-failure outcome выбирается `-compile_failure_mode 0/1/2/3`; default `1` пишет предназначенный `prop_static` даже при отсутствующей variation, `2/3` сохраняют runtime properties для `prop_dynamic_override`/`prop_scalable`, а `0` source-preserving удаляет entity. Все четыре исхода диагностируются как красный `ERROR`.
...
- Для SDK 2013 SP обязательный compiled model set — `.mdl`, `.vvd`, `.dx90.vtx` и `.phy` при наличии collision. `.dx80.vtx` и `.sw.vtx` optional: при наличии они валидируются, публикуются и входят в manifest/fingerprint, но их отсутствие не отклоняет variation. Exit code/строка `Completed` недостаточны: до commit обязательны проверка каждого фактически принятого файла, StudioMDL-представимого managed internal model name (не более 63 ASCII-байт в `name[64]`) и static-prop flag MDL.
...
- Project state хранится под `%LOCALAPPDATA%\PropsScalingRecompiler\projects\<project_id>`; одновременные run одного проекта запрещены project lock. Человекочитаемые отчёты хранятся отдельно под `logs/<project>--<short_id>` и содержат map/UTC timestamp/PID в имени.
- Основные CLI-аргументы остаются `-game/-vmf_in/-vmf_out`; `-compile_failure_mode 0/1/2/3` управляет failed variations с default `1`, а `-debug_cleanup 0/1/2` является отдельным явным destructive debug-переключателем. Старые флаги принимаются только как deprecated с предупреждением.
