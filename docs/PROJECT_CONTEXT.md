...
- будущий расширенный SMD-transform этап для корректировки skeleton/bone transforms и поворотов collision при dynamic-to-static conversion; это отдельная задача со звёздочкой, текущий релиз ограничивается доказанным staging-only bodygroup `blank` workaround.
...
- Выявлен crash StudioMDL при dynamic-to-static QC с `blank` первым option в `$bodygroup`. Для этого узкого случая PSR создаёт только в staging zero-triangle SMD с сохранёнными nodes/skeleton и подставляет его вместо `blank`; option order/index сохраняются, placeholder не публикуется.
