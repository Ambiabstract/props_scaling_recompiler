...
- Добавлена воспроизводимая one-file PyInstaller-сборка для Windows 10/11 x64. Собственный hook исключает неиспользуемую FGD-базу и модули `srctools`; frozen no-op regression проходит без установленного Python и model tools. Текущая сборка `props_scaling_recompiler.exe` имеет размер 11 432 874 байта (10,90 MiB), SHA-256 `F82035D6372F4E0A4653C98B3DA92D65689E27E40635E0C81C4C46BAF6B65F59`, ниже целевого бюджета 16 MiB и жёсткого предела 64 MiB. Тестовый комплект содержит только основной exe и отдельный `third-party/CrowbarCommandLineDecomp.exe`; VPKEdit в комплект не входит.
