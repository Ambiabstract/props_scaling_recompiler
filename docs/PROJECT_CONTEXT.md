...
- Добавлена воспроизводимая one-file PyInstaller-сборка для Windows 10/11 x64. Собственный hook исключает неиспользуемую FGD-базу и модули `srctools`; frozen no-op regression проходит без установленного Python и model tools. Текущий `props_scaling_recompiler.exe` с production runtime regression имеет размер 11 430 185 байт (10,90 MiB), ниже целевого бюджета 16 MiB и жёсткого предела 64 MiB. Тестовый комплект содержит только основной exe и отдельный `third-party/CrowbarCommandLineDecomp.exe`, суммарно 15 483 689 байт; VPKEdit в комплект не входит.
