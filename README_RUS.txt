...
Малый размер контента:

	Поскольку инструмент создаёт поскейленные копии моделей, вам следует учитывать фактор веса новых файлов в контенте мода.
	
	Однако props_scaling_recompiler создаёт копии только геометрии и не создаёт копии материалов/текстур, поэтому значительного увеличения веса файлов проекта ожидать не следует.
	
	На данный момент соотношение копий к мебагайту примерно 3 к 1, т.е. 3 новых модели будут весить примерно 1 мб (включая лоды и коллизии). Это значит, что на 100 мб дополнительного веса вы сможете позволить себе примерно 300 поскейленных версий ассетов (может меньше, а может больше - зависит от самого ассета).
	После запаковки этого контента в VPK вес будет ещё меньше.
...
Известные проблемы:
	...
	1. В некоторых случаях динамические и физические пропы будут иметь некорректный визуал и/или коллизию. Часть этих проблем удастся исправить в будущем, но не все. В некоторых случаях потребуется переделка оригинального ассета.
	
	2. Возможны фатальные ошибки при компиляции некоторых динамических пропов, например "models/props_c17/door02_double.mdl". Получится ли исправить это в будущем - пока что неизвестно.


Дальнейшие планы:

	1. Сущность нового класса - скейлящиеся физические пропы с сохранением корректной коллизии и конвертацией любых пропов в физический проп.
	
	2. Сущность нового класса - скейлящиеся динамические пропы с сохранением корректной коллизии и конвертацией любых пропов в динамический проп.
	
	3. Новая функция покраски для статических пропов, аналог "rendercolor" из новых версий движка.
	
	4. Аналог Static Prop Combine. Сшиватель нескольких моделей в один статик проп. Такой функционал уже имеется в Propper++ для Hammer++, но есть несколько идей как можно улучшить такой функционал.
