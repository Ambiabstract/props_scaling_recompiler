...
OFF_TEXTURE_INDEX   = 0xD0  # int textureindex (file offset from start of studiohdr)
OFF_TXDIR_COUNT     = 0xD4  # int numcdtextures
OFF_TXDIR_INDEX     = 0xD8  # int cdtextureindex (file offset to array of int offsets-to-strings)
OFF_SKINREF_COUNT   = 0xDC  # int numskinref
OFF_SKINFAM_COUNT   = 0xE0  # int numskinfamilies
OFF_SKIN_INDEX      = 0xE4  # int skinindex (file offset to USHORT table)
STUDIOHDR_SIZE_MIN  = 0x1A0 # safe lower bound for header
MSTEXTURE_SIZE      = 64    # sizeof(mstudiotexture_t) in file
OFF_MSTEX_NAMEOFF   = 0x00  # int name_offset (relative to start of this struct)
...
def read_header_fields(f) -> Tuple[int,int,int,int,int,int,int,int]:
    ...
    txdir_count   = read_i32_at(f, OFF_TXDIR_COUNT)
    txdir_index   = read_i32_at(f, OFF_TXDIR_INDEX)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python mdl_skins_and_cdmaterials.py path\\to\\model.mdl")
        sys.exit(2)

    mdl = sys.argv[1]
    try:
        version, textures, families, cdmats = get_skin_data(mdl)
    ...
    # Печать skin families (имена как в MDL)
    for i, fam in enumerate(families):
        print(f"\nSkin {i}:")
        for mat in fam:
            print("  ", mat)

    # (опционально) показать собранные кандидаты путей
    print("\nResolved candidate paths per skin (using $cdmaterials):")
    expanded = build_expanded_paths(families, cdmats)
    for i, fam in enumerate(expanded):
        print(f"\nSkin {i}:")
        for j, candidates in enumerate(fam):
            print(f"  mat[{j}] candidates:")
            for c in candidates:
                print("    ", c)
